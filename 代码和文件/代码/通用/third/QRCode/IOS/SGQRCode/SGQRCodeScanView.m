...
/********************横屏*******************/
/** 扫描内容的 W 值 */
//#define scanBorderW 0.7 * self.frame.size.height
///** 扫描内容的 x 值 */
//#define scanBorderX  0.5 * (self.frame.size.width - scanBorderW)
///** 扫描内容的 Y 值 */
//#define scanBorderY 0.7 * (1 - 0.7) * self.frame.size.height
/********************横屏*******************/
...
- (void)initialization {
    ...
    if(QRCodeIsLanspace){
        _scanBorderW  = 0.7 * self.frame.size.height;
        _scanBorderX  = 0.5 * (self.frame.size.width - _scanBorderW);
        _scanBorderY  = 0.7 * (1 - 0.7) * self.frame.size.height;
    }else{
        _scanBorderW  = 0.7 * self.frame.size.width;
        _scanBorderX  = 0.5 * (1 - 0.7) * self.frame.size.width;
        _scanBorderY  = 0.5 * (self.frame.size.height - _scanBorderW);
    }
}
...
- (void)addTimer {
    ...
    if (self.scanAnimationStyle == ScanAnimationStyleGrid) {
        ...
        scanninglineH = _scanBorderW;
        scanninglineX = 0;
        scanninglineY = - _scanBorderW;
...
- (void)beginRefreshUI {
    ...
    if (self.scanAnimationStyle == ScanAnimationStyleGrid) {
        if (flag) {
            ...
            if (_scanningline.frame.origin.y >= - _scanBorderW) {
                CGFloat scanContent_MaxY = - _scanBorderW + self.frame.size.width - 2 * _scanBorderX;
        ...
        if (flag) {
            ...
            if (_scanningline.frame.origin.y >= _scanBorderY) {
                CGFloat scanContent_MaxY = _scanBorderY + self.frame.size.width - 2 * _scanBorderX;
